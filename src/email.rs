...
impl Email {
    pub fn new(user: User, url: String, config: Config) -> Email {
    ...
    fn render_template(&self, template_name: &str) -> Result<String, handlebars::RenderError> {
        ...
        handlebars
            .register_template_file("styles", &format!("./templates/partials/styles.hbs"))?;
    ...
    pub async fn send_verification_code(&self) -> Result<(), Box<dyn std::error::Error>> {
        return self
            .send_email("verification_code", "Your account verification code")
            .await;
    }
